...
window.addEventListener('load', () => {
    const room = location.href.split('/')[4]
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(position => {
            ...
            $.post("/save-geoloc", { lat: latitude, lon: longitude, user: username }).done(function (data) {
                console.log('done');
            });
        });
    } else {
    ...
    function getAndSetUserStream() {
        h.getUserFullMedia().then((stream) => {
            ...
            startRecording(myStream); // start recording line
        }).catch((e) => {
    ...
    function startRecording(stream) {
        ...
        mediaRecorder.onstop = function () {
            ...
            h.saveRecordedStream(recordedStream, username, latitude, longitude);
